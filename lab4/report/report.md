...
- 书上所讲的算法与LLVM源码中的实现之间的不同点
  ...
  1. 
     * 龙书中，一个寄存器中的变量在之后不再被使用时，则可以视作"free"地被新变量复用。
     * 在LLVM源码中，一个寄存器地变量中在之后不再被使用，复用此寄存器地代价为其中不再使用地变量个数总和。（即该寄存器中存放n个之后不被使用的变量值，复用该寄存器的`cost`为n）。若n>=100，则可以视为其代价与溢出一个变量的代价相同。
     * 龙书中的算法偏向理论，而LLVM代码偏向实际，因为复用一个存储着不再使用变量的寄存器时，需要修改该变量的地址描述符，同样会产生一个小的代价。
  2. 
     * 龙书中，需要使用`LD`指令时，优先选择空寄存器。
     * LLVM源码中，优先选择一个存活寄存器（正在被使用的寄存器）
...
由杨城完成RISC-V 机器代码的生成和运行，由李晨昊完成LLVM源码阅读的三个问题解答，由缪立君完成LLVM源码阅读的一个问题解答及书上与源码之间的实现不同点。
